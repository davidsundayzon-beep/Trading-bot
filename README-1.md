...
- No bot hits 80–100% accuracy on options signals, ever. This one uses
...
- Data comes from `yfinance`, which is free and unofficial. It's fine for
  ...
  paid provider (Tradier, Polygon.io, or your broker's API) — the rest of
  the code doesn't need to change if you keep the same function signatures
  in `data_fetch.py`.
- `tracker.py` logs every signal to a local SQLite database (`signals.db`).
  Build a small follow-up job (or run manually) that checks the price some
  time after each signal and calls `update_outcome()` — that's how you get
  an honest accuracy number to show people, instead of a marketing claim.
