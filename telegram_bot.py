...
def format_forex_signal_message(pair_display: str, signal: dict) -> str:
    ...
    entry_time = now_local + timedelta(minutes=2)

    duration = signal.get("duration_minutes", 2)
    ...
    entry_time_str = f"{entry_time.strftime('%H:%M')} {tz_label}"
    exit_window = f"~{exit_time.strftime('%H:%M')} {tz_label}"

    strength = signal.get("strength", "MODERATE")
    strength_emoji = {"VERY STRONG": "🔥", "STRONG": "💪", "MODERATE": "👍"}.get(strength, "👍")
    ...
    return (
        ...
        f"{strength_emoji} Strength: {strength}\n"
        ...
        f"streak into a wipeout, regardless of signal strength.\n\n"
