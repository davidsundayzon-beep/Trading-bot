...
def score_signal(df: pd.DataFrame, htf_trend: str = "neutral") -> dict:
    ...
    bullish_votes = 0
    bearish_votes = 0
    # MACD and trend fire on almost every scan (whichever side of a
    # crossover price happens to sit on) — they're the "baseline" and
    # were the original problem: signals were firing on these two alone,
    # every time, always scoring exactly 70. RSI, Bollinger Bands, and
    # volume only trigger in genuinely notable conditions (extremes,
    # spikes) — treating these as real "strength confirmations" targets
    # the actual complaint without demanding a rare full 3-vote count.
    bullish_strength_confirmations = 0
    bearish_strength_confirmations = 0

    # RSI momentum
    if latest["rsi"] < 30:
        score += 15
        bullish_votes += 1
        bullish_strength_confirmations += 1
        reasons.append(f"RSI oversold ({latest['rsi']:.1f}) — bullish bias")
    elif latest["rsi"] > 70:
        score -= 15
        bearish_votes += 1
        bearish_strength_confirmations += 1
    ...
    # Real-agreement check: a score can clear 70/30 from just MACD+trend
    # agreeing alone (50+10+10=70) — technically past the threshold, but
    # this was found directly from real trades to be firing EVERY time,
    # always scoring exactly 70, with no genuine extra confirmation.
    #
    # Requiring a flat "3 of 5" count turned out too strict — RSI and
    # Bollinger Bands only trigger in real extreme conditions, so 3+ was
    # rare even during active market hours. Instead: require the baseline
    # (MACD+trend) to agree, AND at least one genuine strength signal
    # (RSI extreme, BB touch, or volume spike) — this directly targets
    # the original problem (MACD+trend alone, nothing else, every time)
    # without demanding a rare full house.
    dominant_votes = bullish_votes if direction == "CALL" else bearish_votes
    dominant_strength = bullish_strength_confirmations if direction == "CALL" else bearish_strength_confirmations

    if direction != "NO_TRADE" and dominant_strength < 1:
        return {
            "direction": "NO_TRADE",
            "score": score,
            "confidence": 0,
            "strength": "WEAK",
            "reasons": [f"Only MACD+trend agree, no real strength confirmation (RSI extreme, BB touch, or volume spike) — too weak on its own"],
            "price": round(float(latest["close"]), 2),
            "duration_minutes": 0,
            "duration_reason": "",
        }
