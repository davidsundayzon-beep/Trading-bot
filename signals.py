...
def score_signal(df: pd.DataFrame, htf_trend: str = "neutral") -> dict:
    ...
    # Raised from 0.03 after real losses at 0.03% and 0.04% showed that
    # floor still let through markets too quiet to trade meaningfully.
    ...
    # Higher-timeframe confirmation: don't fire a scalp against the
    # broader 4H trend, even if the short-term indicators qualify.
    if direction == "CALL" and htf_trend == "bearish":
        return {
            "direction": "NO_TRADE",
            "score": score,
            "confidence": 0,
            "reasons": ["Short-term signal is bullish, but 4H trend is bearish — skipping to avoid trading against the bigger picture"],
            "price": round(float(latest["close"]), 2),
            "duration_minutes": 0,
            "duration_reason": "",
        }
    if direction == "PUT" and htf_trend == "bullish":
        return {
            "direction": "NO_TRADE",
            "score": score,
            "confidence": 0,
            "reasons": ["Short-term signal is bearish, but 4H trend is bullish — skipping to avoid trading against the bigger picture"],
            "price": round(float(latest["close"]), 2),
            "duration_minutes": 0,
            "duration_reason": "",
        }
