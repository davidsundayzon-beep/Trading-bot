...
def suggest_duration_minutes(latest_row) -> tuple:
    ...
    given move sooner, so they get a shorter suggested duration; slower,
    quieter pairs get more time. Bounded to a 3-10 minute range since the
    underlying signal is built from 5-minute candles — going far outside
    that window drifts away from what the signal actually measured.
    Returns (minutes, reason_text).
    """
    atr_pct = (latest_row["atr"] / latest_row["close"]) * 100

    if atr_pct >= 0.15:
        return 3, f"High volatility (ATR {atr_pct:.2f}% of price) — fast mover, shorter window"
    elif atr_pct <= 0.05:
        return 10, f"Low volatility (ATR {atr_pct:.2f}% of price) — slow mover, more time needed"
    else:
        return 5, f"Moderate volatility (ATR {atr_pct:.2f}% of price) — standard window"
